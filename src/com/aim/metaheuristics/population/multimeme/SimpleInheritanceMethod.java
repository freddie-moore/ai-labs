...
public class SimpleInheritanceMethod implements MemeplexInheritanceMethod {
	...
	public void performMemeticInheritance(int parent1, int parent2, int child1, int child2) {
		double parent1Eval = problem.getObjectiveFunctionValue(parent1);
		double parent2Eval = problem.getObjectiveFunctionValue(parent2);

		double p = rng.nextDouble();

		if(parent1Eval < parent2Eval || p < 0.5) {
			problem.copySolution(parent1, child1);
			problem.copySolution(parent1, child2);
		} else {
			problem.copySolution(parent2, child1);
